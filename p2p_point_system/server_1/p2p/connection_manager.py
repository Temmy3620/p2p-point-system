...
class ConnectionManager:
    ...
    def __wait_for_access(self):
        ...
        self.socket.listen(0)#ここで指定した数だけ並列的にリクエストを処理する
        ...
        while True:
            ...
            executor.submit(self.__handle_message, params)#結果を受け取る
    ...
    def __check_peers_connection(self):
        ...
        if changed:
            ...
            msg = self.mm.build(MSG_CORE_LIST, self.port, cl)
            self.send_msg_to_all_peer(msg)#Coreノードリストの登録されているすべてのノードに対してmsgをブロードキャストする
            self.send_msg_to_all_edge(msg)#Coreノードリストの登録されているすべてのノードに対してmsgをブロードキャストする
        self.ping_timer_p = threading.Timer(PING_INTERVAL, self.__check_peers_connection)
        self.ping_timer_p.start()
